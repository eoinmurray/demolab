...
**G10 — Engine.** The Typst publishing code inside the `demolab-cli` package (`main.typ`, `lib.typ`, `build.py`, `style.css`, `favicon.svg`). The heart of the black box (G3); the pieces Typst must read are staged into `.demolab/`.
...
**G21 — Runbook.** An agent procedure shipped with the package (`demolab docs <NAME>` prints its path), fired by a trigger phrase (e.g. *"how do I get started"*). The source of truth for operating the repo.
