...
(`demolab docs HOUSESTYLE` prints it) to your lab. It survives `"update demolab"` (like
`demolab.yaml`); delete it to use the default unchanged.
...
- `<!-- mode: replace -->`: an agent ignores demolab's house style entirely and follows
  **only** this file. The RULES contract (`demolab docs RULES`) still applies; only *taste*
  is overridable.
