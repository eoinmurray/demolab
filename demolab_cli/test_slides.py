...
from demolab_cli import build, slides
...
def test_slides_rejects_web_only_lab(monkeypatch):
    monkeypatch.setattr(build, "pdfs_enabled", lambda: False)
    with pytest.raises(SystemExit, match="PDF publishing is disabled"):
        slides.main()
