...
it — it cites each rule, it doesn't restate it. (There is no `task doctor` — this replaced it,
and folds the toolchain check in as step 0.) It reports; it fixes only what you approve, then
re-runs.
