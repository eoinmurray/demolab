...
- **The repo must be the user's own copy.** If you arrived with just a URL and an empty folder, clone + *degit* it first, so there's **no `origin` remote** pointing at upstream and the upstream Pages workflow is gone: `git clone --depth 1 https://github.com/eoinmurray/demolab . && rm -rf .git .github/workflows/landing.yml && git init && git add -A && git commit -m "Start my lab from demolab"`. If the tree is **already here and scaffolded** (an `install.sh` run brought you), don't re-clone — resume at step 2.
