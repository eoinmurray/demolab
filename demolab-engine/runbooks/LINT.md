...
- **§H9 numbers (read for these — no grep).** Every number a run *produced* — in prose, captions, and tables — traces to `numbers.json` via `#run…` / `#numbers-table(...)`, **never** a typed literal that can drift. Hardcoded run figures are almost always a violation to fix. The only literals allowed are values that are *not* run outputs: a model's fixed coefficients in an equation, a structural count, or a paper's target quoted for comparison (§6.6).
