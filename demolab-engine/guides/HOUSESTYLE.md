...
**H24 — Cite with the helpers, never hand-type.** Inline citations use `#cite(1, 2)` and the reference list uses `#reference-list(((text: […], doi: "10.…"), …))` (RULES §6.2) — never a hand-typed `[1]` or a manually written list. The helper renders the bracketed number, links it to its reference, auto-manages the numbering, and drives the web's hover-popover + new-tab DOI; a typed cite gets none of that, and its number drifts the moment you reorder. This is H9's discipline for citations — the rendering comes from the helper, not the keyboard. Cite claims that lean on someone else's result, method, or data (your own runs are backed by the figure + numbers, not a citation); give each reference author, year, title, and a DOI where one exists. It applies the moment you cite once, on any article — no per-page setup; the popover script ships with every page.
