...
- **Python**: use `uv`. Never call `python` / `python3` directly. Pass dependencies inline with `--with`, e.g. `uv run --with numpy --with matplotlib python src/simulators/simulator/cli.py lif`.
