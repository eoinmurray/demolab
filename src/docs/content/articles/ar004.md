...
> a **tool** runs an experiment and drops a directory of artifacts → a **notebook runner** bundles those artifacts → an **Astro site** publishes a post for each notebook.
...
Open <http://localhost:4321> and click into **nb000**. You're looking at a post generated from the tool run you just executed — the figure and the numbers in the prose came straight out of `src/artifacts/neuron/` via `src/docs/public/notebooks/nb000/`.
...
>
> 1. **Add a tool command** for it. Either add a subcommand to an existing tool under `src/tools/<tool>/tool.py`, or create a new tool directory. Follow "Adding a new notebook" / "Adding a new tool" in `CONTRIBUTORS.md` exactly — write `config.json`, `output.json`, a figure, and pass a `manifest` to `write_output` declaring the headline figure and metrics. Model it on `src/tools/neuron/tool.py`.
...
>
> - **Example tools:** `src/tools/neuron`, `src/tools/mujoco`, `src/tools/playground`
...
- `CONTRIBUTORS.md` — the tool ↔ notebook contract and the procedures for adding notebooks and tools.
