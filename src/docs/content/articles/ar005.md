...
- **Incremental, not lift-and-shift.** Bring experiments across **one at a time**. A research repo doesn't map onto this structure wholesale; each experiment becomes its own tool command + notebook + post.
- **Wrap, don't rewrite.** Keep the user's code where it is. The new `tool.py` is a *thin adapter* that imports and calls their existing functions — you're not reimplementing their science.
...
The aim is for their computation to stay in their modules; the tool just calls it. Two cases:

- **It's an installable package** (has a `pyproject.toml` / `setup.py`): add it as a dependency so you can import it without copying — `uv add <name>` for a PyPI package, or a git/local-path dependency for an unpublished repo — then `uv sync`. Now `import their_package` works from the tool.
- **It's loose scripts/modules:** copy only the modules the chosen experiment needs into the new tool directory (next to `tool.py`), or a shared `src/<their_pkg>/`. When a tool runs via `uv run python src/tools/<tool>/tool.py`, that directory is on the import path, so `from their_module import run_experiment` resolves.
...
## 4. Wrap the first experiment as a tool command

> **Agent:** create `src/tools/<tool>/tool.py` (or add a subcommand to an existing tool), modeled on `src/tools/neuron/tool.py`:
>
> 1. Copy the `setup_run_dir` and `write_output` helpers from `neuron/tool.py` verbatim, adjusting the tool name in `ARTIFACTS_DIR` and the logger.
...
>
> The handler should be thin — parse args, call their code, write the contract files. If you find yourself porting their math, stop and import it instead. Run `uv run python src/tools/<tool>/tool.py <command>` and confirm `src/artifacts/<tool>/<command>/` has the full set (`config.json`, `output.json`, `manifest.json`, the figure, `run.sh`).

## 5. Runner and post

> **Agent:** create the runner `src/notebooks/nbNNN.py` (modeled on `nb000.py`, or `nb002.py` for a tool mix) with its `COMMANDS`, and run it. Then create the post `src/docs/content/notebooks/nbNNN.mdx` — `title` + `date` frontmatter (optional `description`/`collection`/`status`), with values inlined from the generated `numbers.json`, the figure, and a parameter table. Start the site (`cd src/docs && bun run dev`) and open the new post with the user at `http://localhost:4321`. Confirm the figure and numbers match their original code's output.
...
- **Don't fork their science into the contract — wrap it.** The tool is glue; their code is the source of truth.
- **Determinism:** if an experiment uses randomness, thread a `--seed` argument through (see `neuron`'s `net` command) so runs reproduce.
