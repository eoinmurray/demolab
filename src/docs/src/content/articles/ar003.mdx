...
The app (`src/simulators/streamlit_cli/app.py`) exposes the same LIF dynamics as the `neuron lif` CLI command, but every parameter is a slider in the sidebar:
...
```sh
uv run streamlit run src/simulators/streamlit_cli/app.py
```

Streamlit serves on `http://localhost:8501` by default and opens a browser tab. To pick another port:

```sh
uv run streamlit run src/simulators/streamlit_cli/app.py --server.port 3001
```
...
```sh
uv run python src/simulators/neuron/cli.py lif --current 2.5 --duration 100
```
